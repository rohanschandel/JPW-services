...
def init_db():
    tables = [
        "CREATE TABLE IF NOT EXISTS users (id SERIAL PRIMARY KEY, full_name TEXT NOT NULL, email TEXT UNIQUE NOT NULL, hashed_password TEXT NOT NULL, created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP);",
        "CREATE TABLE IF NOT EXISTS todos (id SERIAL PRIMARY KEY, user_id INT DEFAULT 1, title TEXT NOT NULL, completed BOOLEAN DEFAULT FALSE, status TEXT DEFAULT 'pending', created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP);",
        "CREATE TABLE IF NOT EXISTS bookmarks (id SERIAL PRIMARY KEY, user_id INT DEFAULT 1, title TEXT NOT NULL, url TEXT NOT NULL, category TEXT DEFAULT 'General & IT', description TEXT DEFAULT '', created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP);",
        "CREATE TABLE IF NOT EXISTS hr_contacts (id SERIAL PRIMARY KEY, user_id INT DEFAULT 1, hr_name TEXT NOT NULL, company_name TEXT NOT NULL, hr_number TEXT DEFAULT '', email_id TEXT DEFAULT '', location TEXT DEFAULT '', created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP);",
        "CREATE TABLE IF NOT EXISTS projects (id SERIAL PRIMARY KEY, user_id INT DEFAULT 1, title TEXT NOT NULL, project_title TEXT, live_url TEXT NOT NULL, github_url TEXT DEFAULT '', status TEXT DEFAULT 'Live', created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP);",
        "CREATE TABLE IF NOT EXISTS assignments (id SERIAL PRIMARY KEY, user_id INT DEFAULT 1, title TEXT NOT NULL, subject TEXT DEFAULT '', category TEXT DEFAULT 'Assignment / Homework', deadline_date TEXT NOT NULL, date TEXT, status TEXT DEFAULT 'Active', created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP);",
        "CREATE TABLE IF NOT EXISTS exams (id SERIAL PRIMARY KEY, user_id INT DEFAULT 1, title TEXT NOT NULL, exam_name TEXT, exact_date TEXT NOT NULL, date TEXT, status TEXT DEFAULT 'Upcoming', created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP);"
    ]
    for tbl in tables:
        execute_sql(tbl)
...
class handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        try:
            if "todo" in path:
                ...
                items = [{"id": r[0], "title": r[1], "task": r[1], "completed": bool(r[2]), "is_completed": bool(r[2]), "status": r[3]} for r in rows]
    ...
    def do_POST(self):
        ...
        try:
            if "register" in path:
                ...
                title = body.get("task") or body.get("title") or "New Task"
                ...
                return self._send_json(201, {"id": item_id, "title": title, "task": title, "completed": False, "is_completed": False, "status": "pending"})

            elif "hr" in path:
                hr_name = body.get("hr_name") or "HR Name"
                company_name = body.get("company_name") or "Company"
                hr_number = body.get("hr_number") or ""
                ...
                title = body.get("name") or body.get("title") or body.get("project_title") or "Project"
                live_url = body.get("live_url") or "#"
                github_url = body.get("github_url") or ""
                status = body.get("status") or "Live"
                row = execute_sql(
                    "INSERT INTO projects (title, project_title, live_url, github_url, status) VALUES ($1, $2, $3, $4, $5) RETURNING id",
                    [title, title, live_url, github_url, status]
                )
                item_id = row[0][0] if row else int(time.time())
                return self._send_json(201, {"id": item_id, "title": title, "project_title": title, "name": title, "live_url": live_url, "github_url": github_url, "status": status})

            elif "assignment" in path or "deadline" in path:
                title = body.get("title") or "Assignment"
                ...
                title = body.get("exam_name") or body.get("title") or "Exam"
    ...
    def do_PATCH(self):
        ...
        try:
            if "todo" in path:
                ...
                item_id = int(parts[parts.index("todos") + 1]) if "todos" in parts else int(parts[-2])
                execute_sql("UPDATE todos SET completed = NOT completed WHERE id = $1", [item_id])
                return self._send_json(200, {"status": "toggled", "id": item_id})
    ...
    def do_DELETE(self):
        path = self.path.split("?")[0].lower()
        item_id = int([p for p in path.split("/") if p][-1])
